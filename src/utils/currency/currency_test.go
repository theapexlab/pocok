...
var _ = Describe("TrimCurrencyFromPrice", func() {
	When("receives input with price amount and currency symbol", func() {
		It("return price as a string value", func() {
			Expect(currency.TrimCurrencyFromPrice("7000 Ft")).To(Equal("7000"))
			Expect(currency.TrimCurrencyFromPrice("7000 HUF")).To(Equal("7000"))
			Expect(currency.TrimCurrencyFromPrice("25,000.00 HUF")).To(Equal("25,000.00"))
			Expect(currency.TrimCurrencyFromPrice("25,000,000 HUF")).To(Equal("25,000,000"))
			Expect(currency.TrimCurrencyFromPrice("$120")).To(Equal("120"))
			Expect(currency.TrimCurrencyFromPrice("120.55 $")).To(Equal("120.55"))
			Expect(currency.TrimCurrencyFromPrice("120 USD")).To(Equal("120"))
			Expect(currency.TrimCurrencyFromPrice("€300")).To(Equal("300"))
			Expect(currency.TrimCurrencyFromPrice("300 €")).To(Equal("300"))
			Expect(currency.TrimCurrencyFromPrice("300 EUR")).To(Equal("300"))
			Expect(currency.TrimCurrencyFromPrice("€322,50")).To(Equal("322,50"))
		})
	})

	When("receives input without valid price amount ", func() {
		It("returns empty string", func() {
			Expect(currency.TrimCurrencyFromPrice("EUR")).To(Equal(""))
			Expect(currency.TrimCurrencyFromPrice(", USD")).To(Equal(""))
			Expect(currency.TrimCurrencyFromPrice(",00 USD")).To(Equal(""))
			Expect(currency.TrimCurrencyFromPrice(",0.0 HUF")).To(Equal(""))
		})
	})
})
